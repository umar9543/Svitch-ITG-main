...
const TABLE_HEAD = [
  ...
  { id: 'LawNo', label: 'Industry ', width: 120 },
...
export default function CertificateListView() {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(`/dashboard/RiskAnalysis/Certificate/${id}`);
    },
    [router]
  );
                ...
                icon={
                  ...
                  >
                    {['active', 'pending', 'banned', 'rejected'].includes(tab.value)
                      ? tableData.filter((Certificate) => Certificate.status === tab.value).length
                      : tableData.length}
                  </Label>
                }
              />
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (Certificate) => Certificate.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

  if (status !== 'all') {
    inputData = inputData.filter((Certificate) => Certificate.status === status);
  }

  if (role.length) {
    inputData = inputData.filter((Certificate) => role.includes(Certificate.Initiavtive));
  }
