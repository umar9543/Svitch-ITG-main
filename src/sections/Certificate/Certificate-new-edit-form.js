...
export default function CertificateNewEditForm({ currentCertificate }) {
  ...
  const NewCertificateSchema = Yup.object().shape({
  ...
  const defaultValues = useMemo(
    () => ({
      // LawDatabaseID: currentCertificate?.LawDatabaseID || '',
      LawNo: currentCertificate?.LawNo || '',
      LawDescription: currentCertificate?.LawDescription || '',
      Notes: currentCertificate?.Notes || '',
      InitiativeDatabaseID: currentCertificate?.InitiativeDatabaseID || '',
  ...
  const methods = useForm({
    resolver: yupResolver(NewCertificateSchema),
    defaultValues,
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const PostData = { ...data, UserID: currentCertificate?.UserID || userData[0].UserId };
      const UpdatedData = { ...data, LawDatabaseID: currentCertificate?.LawDatabaseID };
      ...
      //     router.push(paths.dashboard.RiskAnalysis.Certificate.root);
      //   } else {
        ...
        <Grid xs={12} md={12}>
          ...
          <Stack alignItems="flex-end" sx={{ mt: 3 }}>
            ...
            >
              ...
              >
                {!currentCertificate ? 'Save Changes' : 'Save Changes'}
              </LoadingButton>
...
CertificateNewEditForm.propTypes = {
  currentCertificate: PropTypes.object,
};
