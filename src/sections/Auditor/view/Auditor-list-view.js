...
import AuditorTableRow from '../Auditor-table-row';
import AuditorTableToolbar from '../Auditor-table-toolbar';
import AuditorTableFiltersResult from '../Auditor-table-filters-result';
...
const TABLE_HEAD = [
  ...
  { id: 'Job', label: ' Job', width: 180 },
  { id: 'Job Location', label: 'Job Location(s)', width: 180 },
  { id: 'Job Location', label: 'Qualification', width: 180 },
  { id: 'Job Location', label: 'Language', width: 180 },
...
export default function AuditorListView() {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(`/dashboard/audit/auditor/${id}`);
    },
    [router]
  );
                ...
                icon={
                  ...
                  >
                    {['active', 'pending', 'banned', 'rejected'].includes(tab.value)
                      ? tableData.filter((Auditor) => Auditor.status === tab.value).length
                      : tableData.length}
                  </Label>
                }
              />
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (Auditor) => Auditor.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

  if (status !== 'all') {
    inputData = inputData.filter((Auditor) => Auditor.status === status);
  }

  if (role.length) {
    inputData = inputData.filter((Auditor) => role.includes(Auditor.Initiavtive));
  }
