...
export default function PreAuditReportNewEditForm({ currentPreAuditReport }) {
  ...
  const defaultValues = useMemo(
    () => ({
      SiteID: currentPreAuditReport?.SiteID || '',
      City: currentPreAuditReport?.City || '',
      SiteManager: currentPreAuditReport?.SiteManager || '',
      SiteAddress: currentPreAuditReport?.SiteAddress || '',
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      reset();
      enqueueSnackbar(currentPreAuditReport ? 'Update success!' : 'Create success!');
      // router.push(paths.dashboard.PreAuditReport.root);
        ...
        <Grid xs={12} md={12}>
          ...
          <Stack alignItems="flex-end" sx={{ mt: 3 }}>
            <LoadingButton type="submit" variant="contained" color="primary" loading={isSubmitting}>
              {!currentPreAuditReport ? 'Save Changes' : 'Save Changes'}
            </LoadingButton>
          </Stack>
        </Grid>
      </Grid>
...
PreAuditReportNewEditForm.propTypes = {
