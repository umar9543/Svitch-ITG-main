...
import WorkshopTableRow from '../Workshop-table-row';
import WorkshopTableToolbar from '../Workshop-table-toolbar';
import WorkshopTableFiltersResult from '../Workshop-table-filters-result';
...
const TABLE_HEAD = [
  ...
  { id: 'LawNo', label: 'Workshop', width: 400 },
  { id: 'LawDescription', label: 'No. of Invitaion', align: 'center', width: 100 },
  { id: 'Notes', label: 'Participation', align: 'center', width: 100 },
  ...
  { id: 'Delete', label: 'Delete', align: 'center', width: 88 },
];
...
export default function WorkshopListView() {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(`/dashboard/RiskAnalysis/Workshop/${id}`);
    },
    [router]
  );
                ...
                icon={
                  ...
                  >
                    {['active', 'pending', 'banned', 'rejected'].includes(tab.value)
                      ? tableData.filter((Workshop) => Workshop.status === tab.value).length
                      : tableData.length}
                  </Label>
                }
              />
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (Workshop) => Workshop.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

  if (status !== 'all') {
    inputData = inputData.filter((Workshop) => Workshop.status === status);
  }

  if (role.length) {
    inputData = inputData.filter((Workshop) => role.includes(Workshop.Initiavtive));
  }
