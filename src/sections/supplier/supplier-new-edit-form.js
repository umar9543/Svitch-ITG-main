...
function SurveyPdfDownloadButton({ row, renderTrigger }) {
  const assessmentId =
    row?.SupplierAssessmentMstID;
  ...
  const handleDownloadPdf = (e) => {
    ...
    if (!assessmentId || loading) return;
    setLoading(true);
    (async () => {
      try {
        let mergedData;
        ...
        if (pdfData?.allQuestions?.length > 0) {
          mergedData = pdfData.allQuestions;
          mstData = pdfData.mstData;
          marksData = pdfData.marksData;
        } else {
          const [infoRes, marksRes, surveyRes] = await Promise.all([
            Get(`GetSupplierAssesmentInformation?SupplierAssessmentMstID=${assessmentId}`),
            Get(`GetSupplierAssessmentMarksList?SupplierAssessmentMstID=${assessmentId}`),
            Get(`GetSurveyResultByAssesmentID?SupplierAssessmentMstID=${assessmentId}`),
          ]);

          const decryptedInfo = decryptObjectKeys(infoRes?.data?.ServiceRes || []);
          mstData =
            infoRes?.data?.ResponseCode === '-2'
              ? {}
              : {
                ...decryptedInfo[0],
                AssessmentDate: decryptedInfo[0]?.AssessmentDate
                  ? new Date(decryptedInfo[0].AssessmentDate)
                  : null,
              };

          const decryptedMarks = decryptObjectKeys(marksRes?.data?.ServiceRes || []);
          marksData = marksRes?.data?.ResponseCode === '-2' ? [] : decryptedMarks;

          const decryptedSurvey = decryptRecursiveObjectKeys(surveyRes?.data || {});
          const rawSections =
            surveyRes?.data?.ResponseCode === '-2'
              ? []
              : Array.isArray(decryptedSurvey)
                ? decryptedSurvey
                : decryptedSurvey?.Data ?? decryptedSurvey?.ServiceRes ?? [];
          const allQuestionsWithMarks = rawSections.map((section) => ({
            ...section,
            Questions: (section?.Questions || []).map((question) => {
              const markData = marksData?.find(
                (m) => m.QuestionnaireMstID === question.QuestionnaireMstID
              );
              return { ...question, Marks: markData?.Marks ?? question?.Marks ?? 0 };
            }),
          }));
          mergedData = allQuestionsWithMarks.map((section) => ({
            ...section,
          ...
          setPdfData({ allQuestions: mergedData, mstData, marksData });
        }

        if (mergedData?.length > 0) {
          const fileName = `${mstData?.SurveyNo || 'Survey'}_${mstData?.CompanyName || 'AnswerSheet'}_${fDateTime(new Date())}.pdf`;
          const doc = (
            <Conrad
              allQuestions={mergedData}
              mstData={mstData || {}}
              marksData={marksData || []}
            />
          );
        ...
        console.error('Error fetching survey PDF data', err);
      } finally {
        setLoading(false);
      }
    })();
  ...
  if (row?.AnswerSheetUrl) {
    return (
      <Link href={row.AnswerSheetUrl} target="_blank" rel="noopener noreferrer">
        {row.AnswerSheetFileName || 'Answer Sheet'}
      </Link>
    );
  }

  if (!assessmentId || assessmentId === '0' || assessmentId === 0) return <>-</>;

  if (loading) {
    return renderTrigger ? (
      renderTrigger({ onClick: undefined, loading: true, label: 'Preparing PDF...' })
    ) : (
...
SurveyPdfDownloadButton.propTypes = {
...
export default function SupplierNewEditForm({
  currentSupplier,
  allCountries,
  ...
  const fetchWorkshops = useCallback(async () => {
    if (!viewMode || !currentSupplier?.VenderLibraryID) return;
    try {
  ...
  }, [viewMode, currentSupplier?.VenderLibraryID]);
  ...
  const GetCountryRisk = async () => {
    try {
      const res = await Get(`GetRiskAnalysisCountryRateing?CountryID=${currentSupplier?.CountryID}`);
      if (res.data.ResponseCode === '100') {
        setCountryRisk(decryptObjectKeys(res.data.ServiceRes));
      }
    } catch (e) {
      console.log('error getting countryRisk', e);
    }
  };
  ...
  const GetIndustryRisk = async () => {
    try {
      const res = await Get(
        `GetRiskAnalysisIndustryRateing?IndustryTypeID=${currentSupplier?.IndustryTypeID || '2'}&CountryID=${currentSupplier?.CountryID}`
      );
      if (res.data.ResponseCode === '100') {
        setIndustryRisk(decryptObjectKeys(res.data.ServiceRes));
      }
    } catch (e) {
      console.log('error getting industryRisk', e);
    }
  };

  const GetAmforiRisk = async () => {
    try {
      const res = await Get(`GetCertiFicateRateingForAmfori?VenderLibraryID=${currentSupplier?.VenderLibraryID}`);
      if (res.data.ResponseCode === '100') {
        setAmforiRisk(decryptObjectKeys(res.data.ServiceRes));
      } else {
        setAmforiRisk([{ Rate: '-', Name: '' }]);
      }
    } catch (e) {
      setAmforiRisk([{ Rate: '-', Name: '' }]);
    }
  ...
  const GetCertificateRating = async () => {
    try {
      const res = await Get(`GetCertiFicateRateingBySupplierID?VenderLibraryID=${currentSupplier?.VenderLibraryID}`);
      if (res.data.ResponseCode === '100') {
        setCertificateRating(decryptObjectKeys(res.data.ServiceRes));
      }
    } catch (e) {
      console.log('error getting certificateRating', e);
    }
  };

  const GetWorkshopScores = async () => {
    try {
      const res = await Get(`GetSupplierPAScore?SupplierID=${currentSupplier?.VenderLibraryID}`);
      if (res.data.ResponseCode === '100') {
        const decrypted = decryptObjectKeys(res.data.ServiceRes);
        setWorkshopScores(decrypted.map((item) => ({ Name: item.PerformanceAreaName?.trim(), Rate: item.Score })));
      }
    } catch (e) {
      console.log('error getting workshop scores', e);
    }
  };

  const GetSurveyScores = async () => {
    try {
      const res = await Get(`GetSupplierPAScoreSurvey?SupplierID=${currentSupplier?.VenderLibraryID}`);
      if (res.data.ResponseCode === '100') {
        const decrypted = decryptObjectKeys(res.data.ServiceRes);
        setSurveyScores(decrypted.map((item) => ({ Name: item.PerformanceAreaName?.trim(), Rate: item.Score })));
      }
    } catch (e) {
      console.log('error getting survey scores', e);
    }
  };

  useEffect(() => {
    if (viewMode && currentSupplier?.VenderLibraryID) {
      Promise.all([GetCountryRisk(), GetIndustryRisk(), GetAmforiRisk(), GetCertificateRating(), GetWorkshopScores(), GetSurveyScores()]);
    }
  }, [viewMode, currentSupplier?.VenderLibraryID]);
  ...
  const InsertVendorDetail = async (insertVendorDetail) => {
    console.log('insertVendorDetail', insertVendorDetail);
    // decrypt array
  ...
  const InsertVenderPersonnel = async (vendorPersonnel) => {
    ...
    try {
      ...
      if (response.data.ResponseCode === '100') {
        console.log('InsertVenderPersonnel success');
      }
    } catch (error) {
  ...
  const InsertVenderSupply = async (VendorSupply) => {
    ...
    try {
      ...
      if (response.data.ResponseCode === '100') {
        console.log('Insert Vender Supply success');
      }
    } catch (error) {
  ...
  const InsertVendorData = async (newdata) => {
    ...
    try {
      ...
      if (response.data.ResponseCode === '100') {
        ...
        currentSupplier
          ? enqueueSnackbar('Supplier Update failed! ', {
            variant: 'error',
          })
          : enqueueSnackbar('Supplier creation failed! ', {
            variant: 'error',
      ...
      enqueueSnackbar('Error while creating API request (InsertVender)', {
        variant: 'error',
      });
    }
  };
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const newdata = {
        ...data,
        // CountryID,
        CustomerID: selectedCustomers || [UserData[0]?.CustomerId],
  ...
  const columns = [...new Set(industryRisk.map((item) => item.Name?.trim()).filter(Boolean))];

  const getRateForColumn = (riskData, riskName) => {
    const found = riskData?.find((risk) => risk.Name?.trim() === riskName);
  ...
  const groupByCertificate = (data) =>
    data.reduce((acc, item) => {
      ...
      if (!certName) return acc;
      if (!acc[certName]) acc[certName] = [];
  ...
  const columnSums = (() => {
    ...
    columns.forEach((col) => {
      ...
      // Base: Basic Risk or amfori Risk
      calcuRiskCategory.forEach((cat) => {
        const v = parseFloat(getRateForColumn(cat.data, col));
        if (!isNaN(v)) sum += v;
      });

      // Certificate adjustments (existing logic)
      Object.values(groupedCertificates).forEach((group) => {
        const v = parseFloat(getRateForColumn(group, col));
        if (!isNaN(v) && (amforiRisk[0]?.Rate == '-' || amforiRisk[0]?.Rate == 'NA')) {
          sum += v;
        }
      });
      ...
      <Grid xs={12} md={12}>
        <Card>
          ...
          <Grid container spacing={1.5} sx={{ p: 2 }}>
            <Grid spacing={1.5} xs={12} md={viewMode ? 8 : 6}>
              <Typography variant="subtitle2">
                Supplier Name <span style={{ color: 'red' }}>*</span>
              </Typography>
              <RHFTextField name="VenderName" placeholder="Legal Business Name" disabled={viewMode} />
            </Grid>
                ...
                <TextField
                  ...
                  inputProps={{
                    style: {
                      ...
                      backgroundColor: isHighRisk ? 'rgba(245, 39, 39, 0.22)' : 'rgba(39, 245, 118, 0.22)',
                      borderRadius: '8px',
                    },
                  }}
                />
          ...
          <Grid spacing={1.5} xs={12} md={12}>
            ...
            <RHFTextField name="Address1" placeholder="14th Street NewYork..." disabled={viewMode} />
          </Grid>
          <Grid spacing={1.5} xs={12} md={12}>
            <Typography variant="subtitle2">Address Line 2</Typography>
            <RHFTextField name="Address2" placeholder="14th Street NewYork..." disabled={viewMode} />
          </Grid>
          ...
          <Grid container spacing={1.5} sx={{ p: 2 }}>
            ...
            <Grid spacing={1.5} xs={12} md={4}>
              ...
              <RHFTextField name="Province" placeholder="Province" disabled={viewMode} />
            </Grid>
            ...
            <Grid spacing={1.5} xs={12} md={4}>
              ...
              <RHFTextField name="City" placeholder="City" disabled={viewMode} />
            </Grid>
          </Grid>
          ...
          <Grid container spacing={1.5} sx={{ p: 2 }}>
            <Grid spacing={1.5} xs={12} md={4}>
              ...
              <RHFTextField name="PhoneNumber" placeholder="+1 234567890" disabled={viewMode} />
            </Grid>
            <Grid spacing={1.5} xs={12} md={4}>
              <Typography variant="subtitle2">Fax</Typography>
              <RHFTextField name="FaxNo" placeholder="123450..." disabled={viewMode} />
            </Grid>
            <Grid spacing={1.5} xs={12} md={4}>
              <Typography variant="subtitle2">Zip Code</Typography>
              <RHFTextField name="ZipCode" placeholder="781211..." disabled={viewMode} />
            </Grid>
          </Grid>
          ...
          <Grid container spacing={1.5} sx={{ p: 2 }}>
            <Grid spacing={1.5} xs={12} md={4}>
              ...
              <RHFTextField name="Website" placeholder="Web Address" disabled={viewMode} />
            </Grid>
            ...
            <Grid spacing={1.5} xs={12} md={4}>
              ...
              <RHFTextField name="OnBoardingEmail" placeholder="Onboarding Email" type="email" disabled={viewMode} />
            </Grid>
          </Grid>
        </Card>
      </Grid>
      ...
      <Grid xs={12} md={12}>
        <Card>
          ...
          <Grid container spacing={1.5} sx={{ p: 1.5 }}>
            <Grid spacing={1.5} xs={12} md={6}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }} gap={1}>
                <RHFTextField name="Capacity" placeholder="Capacity per Month" disabled={viewMode} />
            ...
            <Grid spacing={1.5} xs={12} md={6}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }} gap={1}>
                <RHFTextField name="Annualturnover" placeholder="Turnover per year" disabled={viewMode} />
          ...
          <Grid spacing={1.5} xs={12} md={12}>
            ...
            >
              <Box sx={{ flex: 1 }}>
                <RHFTextField name="BusinessLicenseNumber" placeholder="8791" disabled={viewMode} />
              </Box>
                ...
                {tableData.map((row, index) => (
                  <>
                    <TableRow key={index}>
                      ...
                      <TableCell>{fDate(row.CertificateFrom, 'dd MMM yyyy')}</TableCell>
                      <TableCell>{fDate(row.CertificateTo, 'dd MMM yyyy')}</TableCell>
                      <TableCell>
                        {row?.Path ? (
            ...
            >
              ...
              <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                ...
                >
                  ...
                  {!viewMode && (logo || VenderLogoByID.length > 0) ? (
                    <Button
                      onClick={() => {
                        logo
                          ? setLogo(null)
  ...
  return (
    <>
      ...
      {viewMode && columns.length > 0 && (
        <Grid xs={12} md={12} px={1.5}>
          <Card sx={{ mb: 2 }}>
            <ButtonBase onClick={() => setRiskExpanded((prev) => !prev)} sx={{ width: '100%', textAlign: 'left' }}>
              <Typography
                variant="h6"
                sx={{
                  p: 2,
                  my: 0.5,
                  borderBottom: riskExpanded ? '1px solid #e0e0e0' : 'none',
                  display: 'flex',
                  alignItems: 'center',
                  width: '100%',
                }}
              ...
              <Scrollbar sx={{ my: 3, px: 2 }}>
                <TableContainer>
                  <Table sx={{ '& tbody tr:nth-of-type(odd)': { backgroundColor: '#fafafa' } }}>
                    <TableHead>
                      <TableRow>
                        <TableCell align="left" sx={{ minWidth: 170, fontWeight: 'medium' }}>LkSG</TableCell>
                        {columns.map((col, i) => (
                          <TableCell key={i} align="center" sx={{ minWidth: 150, fontWeight: 'medium' }}>{col}</TableCell>
                        ))}
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {riskCategories.map((category, catIndex) => (
                        <TableRow key={catIndex}>
                          <TableCell align="left" sx={{ fontWeight: 'medium' }}>{category.name}</TableCell>
                          {columns.map((col, colIndex) => {
                            const val = getRateForColumn(category.data, col);
                            return (
                              <TableCell
                                key={colIndex}
                                align="center"
                                sx={{ color: val === '-' || val === 'NA' ? '#777' : parseFloat(val) < 3 ? 'green' : 'red' }}
                              >
                                {val === '-' ? '-' : val === 'NA' ? 'NA' : parseFloat(val).toFixed(0)}
                              </TableCell>
                            );
                          })}
                        </TableRow>
                      ...
                      {certificateRating.length > 0 && (
                        <TableRow sx={{ backgroundColor: '#F4F6F8' }}>
                          <TableCell align="left" sx={{ fontWeight: 'medium', color: '#637381' }}>Certificate Rating</TableCell>
                          {columns.map((_, i) => (<TableCell key={i} />))}
                        </TableRow>
                      )}
                      ...
                      {Object.keys(groupedCertificates).map((certName, certIndex) => (
                        <TableRow key={certIndex}>
                          ...
                          {columns.map((col, colIndex) => {
                            const val = getRateForColumn(groupedCertificates[certName], col);
                            return (
                              <TableCell
                                key={colIndex}
                                align="center"
                      ...
                      {/* Workshop Score row */}
                      <TableRow>
                        <TableCell align="left" sx={{ fontWeight: 'medium' }}>Workshop</TableCell>
                        {columns.map((col, colIndex) => {
                          ...
                          return (
                            <TableCell
                              key={colIndex}
                              align="center"
                              sx={{ color: val === '-' || val === 'NA' ? '#777' : numVal < 3 ? 'green' : 'red' }}
                            >
                      ...
                      {/* Survey Rating row */}
                      <TableRow>
                        <TableCell align="left" sx={{ fontWeight: 'medium' }}>Survey Rating</TableCell>
                        {columns.map((col, colIndex) => {
                      ...
                      <TableRow>
                        <TableCell align="left" sx={{ fontWeight: 'medium' }}>Supplier Risk Rating</TableCell>
                        {columns.map((col, i) => {
                          const val = columnSums[col] || 0;
                          return (
                            <TableCell key={i} align="center" sx={{ color: val < 3 ? 'green' : 'red' }}>
                              {Math.round(val)}
                            </TableCell>
                          );
                        })}
      ...
      <FormProvider methods={methods} onSubmit={viewMode ? (e) => e?.preventDefault?.() : onSubmit}>
        <Grid
          container
          spacing={3}
          component="fieldset"
          ...
          sx={{
            ...
            minWidth: 0,
            padding: viewMode ? 0 : undefined,
            margin: viewMode ? 0 : undefined,
          }}
        >
          {/* {!loading ? ( */}
          {renderDetails}
        ...
        {/* Survey section outside disabled fieldset so Download PDF button works in viewMode */}
        {viewMode && (
          <Grid container spacing={3} sx={{ margin: 0.5 }}>
            <Grid xs={12} md={12}>
              <Card>
...
SupplierNewEditForm.propTypes = {
  ...
  currentCertificate: PropTypes.array,
