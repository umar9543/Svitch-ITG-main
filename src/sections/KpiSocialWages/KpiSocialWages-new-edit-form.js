...
export default function KpiSocialWagesNewEditForm({ currentKpiSocialWages }) {
  ...
  const defaultValues = useMemo(
    () => ({
      SiteID: currentKpiSocialWages?.SiteID || '',
      City: currentKpiSocialWages?.City || '',
      SiteManager: currentKpiSocialWages?.SiteManager || '',
      SiteAddress: currentKpiSocialWages?.SiteAddress || '',
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      reset();
      enqueueSnackbar(currentKpiSocialWages ? 'Update success!' : 'Create success!');
      // router.push(paths.dashboard.KpiSocialWages.root);
        ...
        <Grid xs={12} md={12}>
          ...
          <Stack alignItems="flex-end" sx={{ mt: 3 }}>
            <LoadingButton type="submit" variant="contained" color="primary" loading={isSubmitting}>
              {!currentKpiSocialWages ? 'Save Changes' : 'Save Changes'}
            </LoadingButton>
          </Stack>
        </Grid>
      </Grid>
...
KpiSocialWagesNewEditForm.propTypes = {
