...
export default function QuestionnaireNewEditForm({ currentQuestionnaire }) {
  ...
  const NewQuestionnaireSchema = Yup.object().shape({
  ...
  const defaultValues = useMemo(
    () => ({
      // LawDatabaseID: currentQuestionnaire?.LawDatabaseID || '',
      LawNo: currentQuestionnaire?.LawNo || '',
      LawDescription: currentQuestionnaire?.LawDescription || '',
      Notes: currentQuestionnaire?.Notes || '',
      InitiativeDatabaseID: currentQuestionnaire?.InitiativeDatabaseID || '',
  ...
  const methods = useForm({
    resolver: yupResolver(NewQuestionnaireSchema),
    defaultValues,
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const PostData = { ...data, UserID: currentQuestionnaire?.UserID || userData[0].UserId };
      const UpdatedData = { ...data, LawDatabaseID: currentQuestionnaire?.LawDatabaseID };
      ...
      //     router.push(paths.dashboard.RiskAnalysis.Questionnaire.root);
      //   } else {
        ...
        <Grid xs={12} md={12}>
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              />
              <DesktopDatePicker name="Date" label="Deadline" />
          ...
          <Stack alignItems="flex-end" sx={{ mt: 3 }}>
            ...
            >
              ...
              >
                {!currentQuestionnaire ? 'Save Changes' : 'Save Changes'}
              </LoadingButton>
              <LoadingButton
                type="submit"
                variant="contained"
                ...
                loading={isSubmitting}
              >
                {!currentQuestionnaire ? 'Send Email' : 'Send Email'}
              </LoadingButton>
            </Box>
          </Stack>
        </Grid>
...
QuestionnaireNewEditForm.propTypes = {
