...
import QuestionnaireTableFiltersResult from '../Questionnaire-table-filters-result';
...
const TABLE_HEAD = [
  ...
  { id: 'LawNo', label: 'Questionnaire', width: 400 },
  { id: 'LawDescription', label: 'No. of Invitaion', align: 'center', width: 100 },
  { id: 'Notes', label: 'No. of Submitted', align: 'center', width: 100 },
...
export default function QuestionnaireListView() {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(`/dashboard/RiskAnalysis/Questionnaire/${id}`);
    },
    [router]
  );
    ...
    <>
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <Card>
                ...
                icon={
                  ...
                  >
                    {['active', 'pending', 'banned', 'rejected'].includes(tab.value)
                      ? tableData.filter((Questionnaire) => Questionnaire.status === tab.value).length
                      : tableData.length}
                  </Label>
                }
              />
          ...
          {canReset && (
            <QuestionnaireTableFiltersResult
              filters={filters}
              onFilters={handleFilters}
              //
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (Questionnaire) => Questionnaire.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

  if (status !== 'all') {
    inputData = inputData.filter((Questionnaire) => Questionnaire.status === status);
  }

  if (role.length) {
    inputData = inputData.filter((Questionnaire) => role.includes(Questionnaire.Initiavtive));
  }
