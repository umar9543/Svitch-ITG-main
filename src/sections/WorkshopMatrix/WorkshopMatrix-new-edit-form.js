...
export default function WorkshopMatrixNewEditForm({ currentWorkshopMatrix }) {
  ...
  const NewWorkshopMatrixSchema = Yup.object().shape({
  ...
  const defaultValues = useMemo(
    () => ({
      // LawDatabaseID: currentWorkshopMatrix?.LawDatabaseID || '',
      LawNo: currentWorkshopMatrix?.LawNo || '',
      LawDescription: currentWorkshopMatrix?.LawDescription || '',
      Notes: currentWorkshopMatrix?.Notes || '',
      InitiativeDatabaseID: currentWorkshopMatrix?.InitiativeDatabaseID || '',

      // currentDate: currentDate,
    }),
    [currentWorkshopMatrix]
  );

  const methods = useForm({
    resolver: yupResolver(NewWorkshopMatrixSchema),
    defaultValues,
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const PostData = { ...data, UserID: currentWorkshopMatrix?.UserID || userData[0].UserId };
      const UpdatedData = { ...data, LawDatabaseID: currentWorkshopMatrix?.LawDatabaseID };
      ...
      //     router.push(paths.dashboard.RiskAnalysis.WorkshopMatrix.root);
      //   } else {
        ...
        <Grid xs={12} md={12}>
          ...
          <Stack alignItems="flex-end" sx={{ mt: 3 }}>
            ...
            >
              ...
              >
                {!currentWorkshopMatrix ? 'Save Changes' : 'Save Changes'}
              </LoadingButton>
...
WorkshopMatrixNewEditForm.propTypes = {
  currentWorkshopMatrix: PropTypes.object,
};
