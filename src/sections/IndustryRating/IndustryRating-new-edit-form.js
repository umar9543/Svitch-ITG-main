...
export default function IndustryRatingNewEditForm({ currentIndustryRating }) {
  ...
  const NewIndustryRatingSchema = Yup.object().shape({
  ...
  const GetIndustryType = async () => {
    try {
      const res = await Get(`GetIndustryType?UserID=${userData[0]?.UserID}`);
      const decryptedData = decryptObjectKeys(res.data.ServiceRes);
      setIndustryType(decryptedData);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  useEffect(() => {
    ...
    // console.log('currentIndustryRating', currentIndustryRating);
    if (currentIndustryRating?.length > 0) {
      setPA(currentIndustryRating);
    }
  }, []);
  ...
  const defaultValues = useMemo(
    () => ({
      InitiativeDatabaseID: currentIndustryRating
        ? currentIndustryRating[0]?.InitiativeDatabaseID
        : '',
      IndustryTypeID: currentIndustryRating ? currentIndustryRating[0]?.IndustryTypeID : '',
      CountryID: currentIndustryRating ? currentIndustryRating[0]?.CountryID : '',
      UserID: userData[0]?.UserID,

      // currentDate: currentDate,
    }),
    [currentIndustryRating]
  );

  const methods = useForm({
    resolver: yupResolver(NewIndustryRatingSchema),
    defaultValues,
  });
  ...
  const InsertIndustryMst = async () => {
    ...
    const UpdateMstData = {
      RiskAnalysisIndustryRateingMstID:
        currentIndustryRating && currentIndustryRating[0]?.RiskAnalysisIndustryRateingMstID,
      InitiativeDatabaseID: values?.InitiativeDatabaseID,
      IndustryTypeID: values?.IndustryTypeID,
      CountryID: values?.CountryID,
    };

    const encryptedInsertIndustryMst = Object.assign(
      {},
      ...Object.keys(InsertMstData).map((key) => ({
        [key]: encrypt(InsertMstData[key]),
      }))
    );
    const encryptedUpdateIndustryMst = Object.assign(
      {},
    ...
    setIsPosting(true);
    try {
      const res = currentIndustryRating
        ? await Put(`updateRiskAnalysisIndustryRateingMst`, encryptedUpdateIndustryMst)
        : await Post(`InsertRiskAnalysisIndustryRateingMst`, encryptedInsertIndustryMst);
      if (res.data.ResponseCode == '100') {
        const InsertPADetails = PA?.map((item) => ({
          RiskAnalysisIndustryRateingMstID: decrypt(
            res.data.ServiceRes[0]?.RiskAnalysisIndustryRateingMstID
          ),
          PerformanceAreaID: item.PerformanceAreaID,
          Rate: item.Rate || '0',
        }));

        const UpdateDetailData = PA?.map((item) => ({
          RiskAnalysisIndustryRateingDtlID: item.RiskAnalysisIndustryRateingDtlID,
          PerformanceAreaID: item.PerformanceAreaID,
          Rate: item.Rate || '0',
        }));
        ...
        const encryptedUpdatePADetails = UpdateDetailData.map((X) =>
          Object.assign(
            {},
            ...Object.keys(X).map((key) => ({
              [key]: encrypt(X[key]),
        ...
        try {
          const res = currentIndustryRating
            ? await Put(`updateRiskAnalysisIndustryRateingDtl`, encryptedUpdatePADetails)
            : await Post(`InsertRiskAnalysisIndustryRateingDtl`, encryptedInsertPADetails);
          if (res.data.ResponseCode == '100') {
            currentIndustryRating
              ? enqueueSnackbar('Industry Risk Updated Successfully!', { variant: 'success' })
              : enqueueSnackbar('Industry Risk Added Successfully!', { variant: 'success' });
          ...
          console.error('error in Industry risk details', error);
        }
      } else {
        console.log('res', res.data.ServiceRes);
        enqueueSnackbar('There was an error processing your request!', { variant: 'error' });
        ...
        <Grid xs={12} md={12}>
          ...
          <Stack alignItems="flex-end" sx={{ mt: 3 }}>
            ...
            >
              ...
              >
                {!currentIndustryRating ? 'Save Changes' : 'Save Changes'}
              </LoadingButton>
...
IndustryRatingNewEditForm.propTypes = {
  currentIndustryRating: PropTypes.object,
};
