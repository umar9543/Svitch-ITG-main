...
import FactorMappingTableRow from '../FactorMapping-table-row';
import FactorMappingTableToolbar from '../FactorMapping-table-toolbar';
import FactorMappingTableFiltersResult from '../FactorMapping-table-filters-result';
...
export default function FactorMappingListView() {
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(`/dashboard/RiskAnalysis/FactorMapping/${id}`);
    },
    [router]
  );
    ...
    <>
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <Card>
          ...
          {canReset && (
            <FactorMappingTableFiltersResult
              filters={filters}
              onFilters={handleFilters}
              //
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (FactorMapping) => FactorMapping.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

  if (status !== 'all') {
    inputData = inputData.filter((FactorMapping) => FactorMapping.status === status);
  }

  if (role.length) {
    inputData = inputData.filter((FactorMapping) => role.includes(FactorMapping.Initiavtive));
  }
