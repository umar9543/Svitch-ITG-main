...
export default function CountrySpecificNewEditForm({ currentCountrySpecific }) {
  ...
  const NewCountrySpecificSchema = Yup.object().shape({
  ...
  const defaultValues = useMemo(
    () => ({
      // LawDatabaseID: currentCountrySpecific?.LawDatabaseID || '',
      LawNo: currentCountrySpecific?.LawNo || '',
      LawDescription: currentCountrySpecific?.LawDescription || '',
      Notes: currentCountrySpecific?.Notes || '',
      InitiativeDatabaseID: currentCountrySpecific?.InitiativeDatabaseID || '',

      // currentDate: currentDate,
    }),
    [currentCountrySpecific]
  );

  const methods = useForm({
    resolver: yupResolver(NewCountrySpecificSchema),
    defaultValues,
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const PostData = { ...data, UserID: currentCountrySpecific?.UserID || userData[0].UserId };
      const UpdatedData = { ...data, LawDatabaseID: currentCountrySpecific?.LawDatabaseID };
      ...
      //     router.push(paths.dashboard.RiskAnalysis.CountrySpecific.root);
      //   } else {
        ...
        <Grid xs={12} md={12}>
          ...
          <Stack alignItems="flex-end" sx={{ mt: 3 }}>
            ...
            >
              ...
              >
                {!currentCountrySpecific ? 'Save Changes' : 'Save Changes'}
              </LoadingButton>
...
CountrySpecificNewEditForm.propTypes = {
  currentCountrySpecific: PropTypes.object,
};
