...
export default function IndustryRiskNewEditForm({ currentIndustryRisk }) {
  ...
  const NewIndustryRiskSchema = Yup.object().shape({
  ...
  const defaultValues = useMemo(
    () => ({
      CountryID: currentIndustryRisk?.CountryID || '',
      IndustryRisk: currentIndustryRisk?.IndustryRisk || '',
      Discription: currentIndustryRisk?.Discription || '',
  ...
  const methods = useForm({
    resolver: yupResolver(NewIndustryRiskSchema),
    defaultValues,
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const PostData = { ...data, UserID: currentIndustryRisk?.UserID || userData[0].UserID };
      ...
      const response = await Post(`InsertIndustryRisk`, encryptedPostData);
      if (response.data.ResponseCode === '100') {
        enqueueSnackbar('Industry Risk added successfully');
        reset();
        router.push(paths.dashboard.RiskAnalysis.RiskFactor.IndustryRisk.root);
      } else {
        enqueueSnackbar('Industry Risk creation failed! Please try again.', {
          variant: 'error',
        });
      }
    } catch (error) {
        ...
        <Grid xs={12} md={12}>
          ...
          <Stack alignItems="flex-end" sx={{ mt: 3 }}>
            ...
            >
              ...
              >
                {!currentIndustryRisk ? 'Save Changes' : 'Save'}
              </LoadingButton>
...
IndustryRiskNewEditForm.propTypes = {
  currentIndustryRisk: PropTypes.object,
};
