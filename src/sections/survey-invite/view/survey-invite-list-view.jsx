...
import SurveyInviteTableRow from '../survey-invite-table-row';
import SurveyInviteTableToolbar from '../survey-invite-table-toolbar';
import SurveyInviteTableFiltersResult from '../survey-invite-table-filters-result';
...
const TABLE_HEAD = [
  ...
  { id: 'SurveyNo', label: 'Survey No.', width: 160 },
  { id: 'Context', label: 'Context', width: 280 },
  { id: 'CountryName', label: 'Country' },
  { id: 'BestBefore', label: 'Best Before' },
  { id: 'Status', label: 'Status' },
  // { id: 'Copy', label: 'Copy' },
  { id: 'SendToSupplier', label: 'Mail', align: 'center' },
  { id: 'SupplierAssessmentMstID', label: 'Result', align: 'center' },
];
...
export default function SurveyInviteListView() {
  // Table component Ref
  const tableComponentRef = useRef();

  // Fetching data:
  const [tableData, setTableData] = useState([]);
  const [countrysSet, setCountrysSet] = useState(new Set());
  ...
  const GetPreOnboardListData = async () => {
    try {
      const res = await Get(`GetSurveyInvitationsList?CustomerID=${UserData[0]?.CustomerId}`);
      const decryptedFilteredCountrys = decryptObjectKeys(res.data.ServiceRes);
      const data = decryptedFilteredCountrys.map((x) => ({
        ...x,
        BestBefore: new Date(x.BestBefore),
      }));
      setTableData(data);
    } catch (error) {
      console.log('error getting supplier filtered countrys', error);
    }
  };
  ...
  //     const response = await Get(`GetSurveyInviteing?UserID=${decrypt(userData[0].UserID)}`);
  ...
  useEffect(() => {
    const uniqueCountrys = new Set(tableData.map((obj) => obj.CountryName));
    setCountrysSet(uniqueCountrys);
  }, [tableData]);
  // Convert the Set back to an array for rendering, if needed
  const uniqueCountrysArray = [...countrysSet];
      ...
      {isLoading ? (
        ...
        <Container maxWidth={settings.themeStretch ? false : 'lg'}>
          ...
          <Card>
                  ...
                  icon={
                    <Label
                      ...
                      color={
                        ...
                        ((tab.value === '' || tab.value === 'Not Invited') && 'default') ||
                        'default'
                      }
                    >
            ...
            <SurveyInviteTableToolbar
              filters={filters}
              onFilters={handleFilters}
              roleOptions={uniqueCountrysArray}
              tableRef={tableComponentRef.current}
            />

            {canReset && (
              <SurveyInviteTableFiltersResult
                filters={filters}
                onFilters={handleFilters}
                //
  ...
  if (name) {
    inputData = inputData.filter(
      (onboarding) =>
        ...
        onboarding.SurveyNo.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        onboarding.Context.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
