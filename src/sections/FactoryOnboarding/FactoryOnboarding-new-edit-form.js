...
export default function FactoryOnboardingNewEditForm({ currentFactoryOnboarding }) {
  ...
  const NewFactoryOnboardingSchema = Yup.object().shape({
  ...
  const defaultValues = useMemo(
    () => ({
      SiteID: currentFactoryOnboarding?.SiteID || '',
      City: currentFactoryOnboarding?.City || '',
      SiteManager: currentFactoryOnboarding?.SiteManager || '',
      SiteAddress: currentFactoryOnboarding?.SiteAddress || '',

      // currentDate: currentDate,
    }),
    [currentFactoryOnboarding]
  );

  const methods = useForm({
    resolver: yupResolver(NewFactoryOnboardingSchema),
    defaultValues,
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      reset();
      enqueueSnackbar(currentFactoryOnboarding ? 'Update success!' : 'Create success!');
      // router.push(paths.dashboard.FactoryOnboarding.root);
  ...
  return (
    <FormProvider methods={methods} onSubmit={onSubmit}>
      <Grid container spacing={3}>
        <Grid xs={12} md={12} display="flex" flexDirection="column" gap={3}>
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              <RHFTextField name="User ID" label=" User ID" />
              <RHFTextField name="Pre Audit No." label=" Pre Audit No." />
              <DesktopDatePicker label="Pre Audit Date"  format="dd/MM/yyyy"/>
          ...
          <Stack alignItems="flex-end" sx={{ mt: 3 }}>
            <LoadingButton type="submit" variant="contained" color="primary" loading={isSubmitting}>
              {!currentFactoryOnboarding ? 'Save Changes' : 'Save Changes'}
            </LoadingButton>
          </Stack>
        </Grid>
      </Grid>
...
FactoryOnboardingNewEditForm.propTypes = {
  currentFactoryOnboarding: PropTypes.object,
};
